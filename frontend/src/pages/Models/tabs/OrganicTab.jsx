...
const OrganicTab = ({ data = [], updateData }) => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Organic Marketing Events</CardTitle>
            </div>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>
                    {selectedIndex >= 0 ? 'Edit Organic Event' : 'Create Organic Event'}
                  </DialogTitle>
                </DialogHeader>
                {editingEvent && (
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder="e.g., Blog SEO Traffic"
                          required
                        />
                      </div>
                      <div className="space-y-2">
                    ...
                    <div className="space-y-4">
                      <h3 className="font-semibold">Traffic & Conversion Metrics</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2">
                          <div className="flex items-center gap-1">
                            <Label htmlFor="dailyOrganicViews">Daily Organic Views</Label>
                            <Info className="h-3 w-3 text-muted-foreground" />
                          </div>
                          <Input
                            id="dailyOrganicViews"
                            type="number"
                            value={editingEvent.dailyOrganicViews}
                            onChange={(e) => {
                              const value = parseInt(e.target.value);
                              setEditingEvent({ ...editingEvent, dailyOrganicViews: isNaN(value) ? 0 : value });
                            }}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-1">
                            <Label htmlFor="viewToLeadRate">View to Lead Rate</Label>
                            <Info className="h-3 w-3 text-muted-foreground" />
                          </div>
                          <Input
                            id="viewToLeadRate"
                            ...
                            min="0"
                            max="100"
                            value={editingEvent.viewToLeadRate * 100}
                            onChange={(e) => {
                              const value = parseFloat(e.target.value);
                              const validValue = isNaN(value) ? 0 : value;
                              const clampedValue = Math.max(0, Math.min(100, validValue));
                              setEditingEvent({ ...editingEvent, viewToLeadRate: clampedValue / 100 });
                            }}
                            placeholder="2"
                          />
                          <p className="text-xs text-muted-foreground">As percentage (e.g., 2 for 2%)</p>
                        </div>
                        <div className="space-y-2">
                          <div className="flex items-center gap-1">
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              const validValue = isNaN(value) ? 0 : value;
                              const clampedValue = Math.max(0, Math.min(100, validValue));
                              setEditingEvent({ ...editingEvent, leadToSaleRate: clampedValue / 100 });
                            }}
                            placeholder="8"
                          />
                          <p className="text-xs text-muted-foreground">As percentage (e.g., 8 for 8%)</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-muted/50 p-4 rounded-lg">
                      <h4 className="font-medium mb-2">Projected Daily Performance</h4>
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="text-muted-foreground">Expected Daily Leads:</span>
                          <span className="ml-2 font-medium">
                            {(editingEvent.dailyOrganicViews * editingEvent.viewToLeadRate).toFixed(1)}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Expected Daily Sales:</span>
                          <span className="ml-2 font-medium">
                            {(editingEvent.dailyOrganicViews * editingEvent.viewToLeadRate * editingEvent.leadToSaleRate).toFixed(2)}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Overall Conversion:</span>
                          <span className="ml-2 font-medium">
                            {(editingEvent.viewToLeadRate * editingEvent.leadToSaleRate * 100).toFixed(3)}%
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        <CardContent>
          {data.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="font-semibold mb-2">No Organic Events</h3>
              <p className="text-muted-foreground">Create organic marketing channels to capture zero-cost traffic.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Expected Daily Sales</TableHead>
              ...
              <TableBody>
                {data.map((event, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{event.dailyOrganicViews.toLocaleString()}</TableCell>
                    <TableCell>{(event.viewToLeadRate * event.leadToSaleRate * 100).toFixed(3)}%</TableCell>
                    <TableCell>
                      {(event.dailyOrganicViews * event.viewToLeadRate * event.leadToSaleRate).toFixed(2)}
                    </TableCell>
                    <TableCell>
